...
def ir_mtm_calc(deals, St_d, d):
    ...
    deals.loc[(deals['start_date'] <= deals['forward_date']) | (deals['fixed_deal'] == 'Y'),
                    'cash_flow'] = deals['amt'] * deals['rate'] * (deals['mat_date'] - (deals['start_date'])).dt.days
    deals.loc[(deals['fixed_deal'] == 'N') &
                (deals['start_date'] > deals['forward_date']),
                    'cash_flow'] = deals['amt'] * deals['forward_rate'] * (deals['mat_date'] - (deals['start_date'])).dt.days
...
#get market data
data_ir_rub = pd.read_excel('/Users/mihailzaytsev/Desktop/Диссер_мага/market_data_gcurve_key.xlsx', engine='openpyxl')
data_ir_rub['tradedate'] = pd.to_datetime(data_ir_rub['tradedate']).dt.date
data_ir_usd = pd.read_excel('/Users/mihailzaytsev/Desktop/Диссер_мага/market_data_usd_curve.xlsx', engine='openpyxl')
...
def volatility_rescaling(returns, wind):
    ...
    rescaled_returns = rescaled_returns.fillna(0.0001)
    return rescaled_returns

#plt.plot(df_log_returns['RUB_IR_0.25'])
#plt.show()

return_rescaling = True

if return_rescaling == True:
    df_log_returns = volatility_rescaling(df_log_returns, wind = 20)
...
index_list = [1, 4, 13, 26, 52, 323, 521]
